    ...
    public partial class BBL_test : Form
    {
        ...
        public string createGPGFile(string Type, string D, string F)
        {
            ...
            if (System.IO.File.Exists(filepathOut))
            {
                ...
                string strCmdText;
                strCmdText = "move /Y" + filepathOut + " " + filepathCopyTo;

                strCmdText = strCmdText.Replace("\\\\", "\\");

                System.Diagnostics.Process.Start("CMD.exe", strCmdText);

                return "GPG File has been created " + filepathCopyTo + " || " + strCmdText;
            }
            else
        ...
        private string bulkCopy1()
        {
            ...
            //BBL_K2_CUSTOMERS
            string queryStr = "SELECT * FROM [Barcode_DEV].[dbo].[testTR] ";

            SqlCommand CMTTcommand = new SqlCommand(queryStr, conn);
            conn.Open();
            SqlDataReader reader = CMTTcommand.ExecuteReader();
            ...
            if (reader.HasRows)
            {
                while (reader.Read())
                {
                   ...
                   query += "INSERT INTO [dbo].[test55]([Name],[LName],[Email],[number])VALUES('t11','aa', '' ,0)";
                }
            }
            reader.Close();

            string queryStr2 = "SELECT TOP (1)  * FROM [Barcode_DEV].[dbo].[testTR] ";

            SqlCommand CMTTcommand2 = new SqlCommand(queryStr2, conn);
 
            SqlDataReader reader2 = CMTTcommand2.ExecuteReader();
            ...
            if (reader2.HasRows)
            {
                while (reader2.Read())
                {
                    ...
                    query += "INSERT INTO [dbo].[test55]([Name],[LName],[Email],[number])VALUES('t22','aa', '' ,0)";
                }
            }
            reader2.Close();
            conn.Close();
        ...
        private string bulkCopy()
        {
            ...
            if (setting != null)
            {
                connString = setting.ConnectionString + "MultipleActiveResultSets=True;";
            }
            connString = connString + "MultipleActiveResultSets=True;";
            SqlConnection conn = new SqlConnection(connString);
            //BBL_K2_CUSTOMERS
            string queryStr = "SELECT TOP (1)  * FROM [Barcode_DEV].[dbo].[testTR] ";
            ...
            if (reader.HasRows)
            {
                ...
                while (reader.Read())
                {
                    custsupport = "SELECT TOP (1)  * FROM [Barcode_DEV].[dbo].[testTR] ";

                    SqlCommand commandCustSupport = new SqlCommand(custsupport, conn);  
                    SqlDataReader readerCustSupport = commandCustSupport.ExecuteReader();

                    while (readerCustSupport.Read())
                    {
                        custsupport = readerCustSupport[0].ToString().Replace("'", "''");
                    }
                ...
                //dt.Load(reader);
                res = CopyData(conn, "[Barcode_DEV].[dbo].[test55]", dataTable);
                if (res == "Pass") Console.WriteLine("Data copied to BBL_K2_CUSTOMERS SQL Server using SQLBulkCopy, -- FINISHED --");
                else return "Error: Fail to data interfaced to BBL_K2_CUSTOMERS[" + res + "]";
        ...
        public string AssignOwnerCash(int lot)
        {
            ...
            try
            {
                ...
                if (readerCustTT.HasRows)
                {
                    ...
                    if (res == "Pass")
                    {
                        Console.WriteLine("SQLBulkCopy, -- FINISHED --");
                    }
                    else
                    {
